  ...
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column label="编号" sortable="custom" prop="id" fixed="left" width="80">
        <template slot-scope="scope">
          <router-link :to="{ path: '/commodityManagement/good/goodDetail', query: { id: scope.row.goodId }}" target="_blank" style="width:300px;"> {{ scope.row.goodId }}</router-link>
        </template>
      </el-table-column>
      <el-table-column label="图片" width="100">
        <template slot-scope="scope">
          <el-image :src="scope.row.imgsURL[0]" :preview-src-list="[ scope.row.imgsURL[0] ]" style="width:80px;height:80px;"/>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          <div class="drawing">
            <div class="right">
              ...
              <div v-if="scope.row.price.length > 1">¥ {{ scope.row.price[0] | 1000 }} - {{ scope.row.price[1] | 1000 }}</div>
              <div v-else>¥ {{ scope.row.price[0] | 1000 }}</div>
            </div>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="分类">
        <template slot-scope="scope">
          <span>{{ scope.row.goodType ? scope.row.goodType : '无' }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" class-name="small-padding fixed-width" width="148" fixed="right">
        <template slot-scope="scope">
          <router-link :to="{ path: 'GoodEdit', query: { id: scope.row.goodId }}">
            <el-tooltip class="item" effect="dark" content="编辑" placement="top-start">
              <el-button type="primary" icon="el-icon-edit" circle/>
            </el-tooltip>
          </router-link>
          <el-tooltip v-if="scope.row.onSale !== 1" class="item" effect="dark" content="立即发布" placement="top-start">
            <el-button :loading="formLoading" type="success" icon="el-icon-sell" circle @click="handleState(scope.row, 1)"/>
          </el-tooltip>
          <el-tooltip v-else class="item" effect="dark" content="放入库存" placement="top-start">
            <el-button :loading="formLoading" type="info" icon="el-icon-sold-out" circle @click="handleState(scope.row, 0)"/>
          </el-tooltip>
    ...
    <div class="pagination-operation">
      <div class="operation">
        ...
        <el-button v-if="listQuery.activeIndex === '3'" :loading="formLoading" size="mini" type="success" @click="handleAllState()">立即发布</el-button>
        <el-button v-else-if="listQuery.activeIndex === '2'" :loading="formLoading" size="mini" type="info" @click="handleAllState(1)">放入库存</el-button>
  ...
  data() {
    return {
      ...
      listQuery: {
        ...
        pageSize: 10,
        pageNow: this.$route.query.page ? Number(this.$route.query.page) : 1,
        goodId: ''
      },
      temp: {},
      categories: [],
  ...
  methods: {
    getList() {
      ...
      getList(this.listQuery).then(response => {
        this.list = response.data.commodities
    ...
    getCateList() {
      getCateList().then(response => {
        this.categories = response.data.types.map(t => ({
          id: t.goodTypeId,
          value: t.goodTypeId,
          pid: 0,
          label: t.name
    ...
    changeCategories(id) {
      ...
      this.listQuery.searchType =
        id.length ? this.categories.find(c => c.id === id[0]).label : ''
      this.handleFilter()
    },
    ...
    handleState(row, type) { // 变更状态
      ...
      this.$confirm(title, this.$t('hint.hint'), {
        ...
        const formData = createFormData(row)
        formData.set('onSale', type)
