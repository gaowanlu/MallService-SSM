  ...
  <div class="login-container">
    ...
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <div style="position:relative">
        <!--<div class="tips">
          <span>{{ $t('login.id') }} : admin</span>
          <span>{{ $t('login.password') }} : {{ $t('login.any') }}</span>
        </div>
        <div class="tips">
          <span style="margin-right:18px;">{{ $t('login.id') }} : editor</span>
          <span>{{ $t('login.password') }} : {{ $t('login.any') }}</span>
        </div>
...
export default {
  ...
  data() {
    const validateUsername = (rule, value, callback) => {
      if (!value) {
        callback(new Error(this.$t('errorMsg.logoUserNmae')))
      } else {
        callback()
      }
    }
    const validatePassword = (rule, value, callback) => {
      if (value.length < 4) {
        callback(new Error(this.$t('errorMsg.logoPasswordLength')))
      } else {
        callback()
      }
    }
  ...
  methods: {
    ...
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        ...
        if (valid) {
          this.loading = true
          loginByEmail(this.loginForm).then(() => {
