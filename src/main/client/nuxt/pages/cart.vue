    ...
    <template v-if="empty">
      <div class="cart container">
        ...
        <div class="instructions">
          ...
          <div class="login" v-if="!$store.state.hasLogin">
            登录后将显示您之前加入的商品
          </div>
          <div class="operation">
            <template v-if="$store.state.hasLogin">
              <NuxtLink class="li" to="/category/list"
                ><el-button type="danger">马上去购物</el-button></NuxtLink
              >
            </template>
            <template v-else>
              <NuxtLink class="li" to="/pass/login"
                ><el-button type="danger">立即登录</el-button></NuxtLink
              >
              <NuxtLink class="li" to="/category/list"
                ><el-button type="danger" plain>马上去购物</el-button></NuxtLink
              >
            </template>
          </div>
        </div>
      ...
      <div class="cart-list container" v-loading="loading">
        ...
        >
          ...
          <el-table-column label="操作" width="80" align="center">
            <template slot-scope="scope">
              ...
              >
                <el-button
                  @click.stop="deleteCartItem(scope.$index)"
                  size="mini"
                  type="danger"
                  icon="el-icon-delete"
                  circle
        ...
        <div class="cart-operation">
          <div class="left">
            ...
            <span
              >共
              <span class="number">{{ cartList.length }}</span> 件商品，已选择
        ...
        <template v-if="invalidGood.length">
          ...
          <el-table :data="invalidGood" ref="invalidTable" class="invalidTable">
            <el-table-column align="center" width="100">
              <template slot-scope="scope">
                <NuxtLink
                  :to="{
                    path: '/product/detail',
                    query: { id: scope.row.good_id }
                  }"
                >
                  <el-image
                    class="image"
            ...
            <el-table-column label="操作" width="80" align="center">
              <template slot-scope="scope">
                ...
                >
                  <el-button
                    @click="deleteInvalidGood(scope.$index)"
                    size="mini"
                    type="danger"
                    icon="el-icon-delete"
                    circle
